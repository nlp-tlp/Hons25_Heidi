...
st.markdown("This chat functions as a basic search for vector embeddings using cosine similarity. The embeddings are retrieved from an existing ChromaDB vector database. Current embeddings have been generated from the `GloVe` word embedding model using mean pooling to allow for sentences to be embedded.")

# Chat and settings history
if "embeddings_history_glove" not in st.session_state:
    st.session_state.embeddings_history_glove = []
    ...
    with st.chat_message("assistant", avatar=":material/keyboard_return:"):
        with st.spinner("Searching..."):
            ...
            records = embeddings_search(
                search=search,
                embeddings_type="glove mean-pooling",
                k=st.session_state.embeddings_sentence_k,
                threshold=st.session_state.embeddings_sentence_threshold
            )
